...
/// 종료 후 배치 화자 재계산.
///
/// 실시간 LS-EEND 라벨은 근사였고, 후반부 '정밀 재처리'는 세션 원음 녹음이 생기기 전엔 재료가 없었다.
...
/// 실시간 경로(`MicTranscriptionEngine`)는 한 줄도 수정하지 않는다 — 신규 파일 + 명시적 버튼 실행만(불변식: 기존 경로를 조용히 고쳐쓰지 않는다).
/// 라이브 라벨과 배치 재계산을 둘 다 유지한다 — 라이브는 '지금 누가', 배치는 '기록의 정확'.
